...
function ScheTeacher() {
  ...
  useEffect(() => {
    console.log(subject, "selectedYears ");
  }, [subject]);
      ...
      for (let i = 0; i < teacher.length; i++) {
        for (let j = 0; j < subject.length; j++) {
          ...
          // }
            ...
            console.log(teacher1.subject_name)
            console.log(subject1.subject_name)
            Swal.fire({
              icon: "error",
              title: "ไม่สามารถลงทะเบียนรายวิชาได้",
              text: `มีวิชาอื่นของคุณ ที่ลงทะเบียนในวันและเวลาที่เลือก อยู่ในระบบแล้ว`, // Assuming teacher array has at least one item
  ...
  const renderScheteacher = (value, index) => {
    ...
    const handleSelectChange = (event, index, subject_major_id) => {
      ...
      // อัปเดตค่าของ subject_major_id ในอาร์เรย์ subject
      setSubject((prevState) => {
        const updatedSubject = [...prevState];
      ...
      <div className="choseTeacher" key={index}>
        ...
        <div>
          <select
            ...
            value={subject[index].subject_major_id}
          >
            <option value="" >
              choose
            </option>
